 ...
 * public/{route_prefix}/app, servis directement par le serveur web, jamais
 * par cette route (ce chemin doit matcher `app.baseURL` du build Nuxt, cf.
 * ModelbaseServiceProvider). Le routage interne du SPA (Vue Router,
 * historique HTML5) prend ensuite le relais côté navigateur, quel que soit
 * le sous-chemin demandé (routes/web.php capture tout sous {prefix}/app).
 */
class SpaController extends Controller
{
    public function __invoke(): Response
    {
        $index = public_path(config('modelbase.route_prefix').'/app/index.html');
