...
<exigence id="EX-402">Pour chaque item listé, le système affiche un aperçu des valeurs de toutes les colonnes exposées par le modèle (EX-422).</exigence>

<exigence id="EX-403">Le listing des items est paginé ; l'utilisateur peut naviguer entre les pages.</exigence>

<exigence id="EX-469">Dans l'aperçu d'une cellule (EX-402), une valeur dont le rendu dépasse trois lignes est tronquée à ces trois lignes, avec une ellipse signalant la troncature, afin de conserver des lignes de tableau de hauteur maîtrisée.</exigence>
...
<exigence id="EX-470">Le filtre (EX-432 à EX-434) et le tri (EX-435, EX-436) sont également disponibles sur chaque tableau d'objets liés (EX-425 à EX-431), au même titre que sur le listing standard.</exigence>

<exigence id="EX-471">Le filtre et le tri d'un tableau d'objets liés s'appliquent indépendamment de ceux des autres tableaux d'objets liés et du listing standard : chacun conserve son propre état, à l'image de la pagination (EX-456).</exigence>

<exigence id="EX-472">Le filtre et le tri d'un tableau d'objets liés ne portent que sur les colonnes exposées par le modèle lié (EX-427) ; les attributs propres à la table pivot d'une relation belongsToMany en sont exclus, au même titre qu'ils ne sont pas affichés dans l'aperçu.</exigence>

<exigence id="EX-473">Si le modèle ciblé par une relation appartient à une connexion indisponible (EX-431), aucun filtre ni tri n'est proposé pour le tableau correspondant.</exigence>
...
<exigence id="EX-427">Chaque tableau affiche un aperçu des valeurs de toutes les colonnes exposées par le modèle lié, selon la même logique d'aperçu que le listing standard d'un modèle (EX-402, EX-469).</exigence>
...
<exigence id="EX-465">Lors de la modification d'un item existant (EX-413), seules les colonnes dont l'utilisateur a effectivement changé la valeur par rapport à celle initialement chargée sont transmises au serveur ; une colonne non touchée n'est jamais incluse dans la requête de modification.</exigence>

<exigence id="EX-466">Une colonne non touchée par l'utilisateur n'est jamais rejetée ni ne déclenche d'erreur de validation (EX-417), quelle que soit sa valeur actuellement enregistrée.</exigence>

<exigence id="EX-467">Pour une colonne de type JSON, la comparaison entre valeur initiale et valeur saisie porte sur le contenu structuré, indépendamment de sa représentation textuelle (formatage, ordre des clés) dans l'éditeur.</exigence>

<exigence id="EX-468">Tant qu'aucune colonne n'a été modifiée par rapport aux valeurs initialement chargées, la validation du formulaire de modification n'est pas proposée à l'utilisateur.</exigence>
...
<limite>⚠️ Point ouvert : cette exigence réduit le risque qu'une modification écrase une valeur qu'un autre utilisateur aurait changée entre-temps sur une colonne différente du même item, mais ne détecte ni ne bloque une modification concurrente portant sur la même colonne (pas de verrouillage optimiste ni d'horodatage de version) — ce mécanisme de détection de conflit reste hors périmètre.</limite>
...
<limite>À date, le filtre d'affichage des items soft-deleted et les actions de restauration/suppression définitive ne s'appliquent qu'au listing standard et à la fiche détail d'un item ; les tableaux d'objets liés (EX-425 à EX-431) affichent les objets liés tels que renvoyés par la relation Eloquent, sans filtre de visibilité dédié.</limite>
...
<limite>⚠️ Si le pilote Scout configuré par l'application hôte est asynchrone (mise en file d'attente), la mise à jour effective de l'index peut ne pas être immédiate ; le plug-in ne fait que déclencher la synchronisation, sans en garantir le délai de traitement.</limite>
