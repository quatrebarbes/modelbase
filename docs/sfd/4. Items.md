...
<limite>Ce comportement différentiel ne s'applique qu'à la modification d'un item existant (EX-413) ; la création d'un item (EX-412) transmet l'ensemble des colonnes renseignées, la notion de valeur « non modifiée » n'ayant pas de sens en l'absence d'item préexistant.</limite>

<limite>Une colonne en lecture seule (technique, EX-416, ou non-fillable, EX-464) n'étant jamais modifiable par l'utilisateur, elle n'est par construction jamais transmise par cette modification différentielle.</limite>

<limite>Le rétablissement manuel d'une colonne à sa valeur initiale, après une première modification annulant ainsi le changement, la fait sortir du périmètre de la requête transmise au même titre qu'une colonne jamais touchée : la comparaison porte sur la valeur courante par rapport à la valeur initiale, pas sur un historique des saisies de l'utilisateur.</limite>

<limite>Point ouvert : cette exigence réduit le risque qu'une modification écrase une valeur qu'un autre utilisateur aurait changée entre-temps sur une colonne différente du même item, mais ne détecte ni ne bloque une modification concurrente portant sur la même colonne (pas de verrouillage optimiste ni d'horodatage de version) — ce mécanisme de détection de conflit reste hors périmètre.</limite>
