...
Ce module permet à un utilisateur de l'application hôte de parcourir, créer, modifier et supprimer les items (enregistrements) d'une table/modèle sélectionnée (module 3). Il s'agit de la dernière étape du parcours : Base de données (module 2) → Tables/Modèles (module 3) → Items (ce module).

Un « item », au sens métier de ce module, correspond à un enregistrement d'une table/modèle. Chaque item porte une valeur pour chacune des colonnes définies par sa table.
...
classDiagram
    ...
    Table "1" --> "0..*" Column : possède
    Column --> ColumnType : type
    Column "0..*" --> "0..1" Table : référence (si FOREIGN_KEY)
```
...
<exigence id="EX-401">Le système liste les items d'une table/modèle sélectionnée sous forme de tableau, une ligne par item.</exigence>
...
<exigence id="EX-404">Si la table/modèle sélectionné ne contient aucun item, le listing affiche un message indiquant l'absence d'item disponible, sans erreur.</exigence>
...
<exigence id="EX-408">Une valeur de colonne de type clé étrangère est affichée comme un lien de navigation vers l'item référencé, dans sa propre table.</exigence>
...
<exigence id="EX-411">Depuis la fiche détail d'un item, l'utilisateur peut revenir au listing des items de la table.</exigence>

## Création et modification d'un item

<exigence id="EX-412">L'utilisateur peut créer un nouvel item dans une table, en renseignant une valeur pour chacune de ses colonnes.</exigence>
...
<exigence id="EX-415">La modification d'une valeur de colonne de type clé étrangère se fait par sélection d'un item existant dans la table référencée.</exigence>
